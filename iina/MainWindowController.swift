...
class MainWindowController: NSWindowController, NSWindowDelegate {
  ...
  func blackOutOtherMonitors() {
    ...
    for screen in screens {
      ...
      blackWindow.level = NSWindow.Level(NSWindow.Level.mainMenuWindowLevel.rawValue + 1)
  ...
  func setWindowFloatingOnTop(_ onTop: Bool) {
    ...
    if onTop {
      window.level = NSWindow.Level(NSWindow.Level.floatingWindowLevel.rawValue - 1)
    } else {
      window.level = .normalWindowLevel
    }
